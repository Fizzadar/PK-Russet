...
var editor = {
	...
	start: function() {
		...
		//draw blocks
		setTimeout( function() { editor.drawBlocks(); }, 500 );
	...
	showTileFolder: function( folder ) {
		...
		$.each( editor.tiles[folder], function( category, tiles ) {
			...
			for( var i = 0; i < tiles.length; i++ ) {
				tilediv.append( '<img class="tile" id="' + folder + '/' + category + '/' + tiles[i] + '" src="inc/img/tiles/enlarged/' + folder + '/' + category + '/' + tiles[i] + '.png" />' );
			}
		});
	...
	selectTile: function( tile ) {
		this.tile = tile;
		this.preview.attr( 'src', 'inc/img/tiles/enlarged/' + tile + '.png' );
		this.preview.show();
	...
	deselect: function() {
		this.tile = null;
		this.preview.attr( 'src', 'inc/img/blank.png' );
	},
	...
	export: function() {
		if( $( '#edit_export' ).css( 'display' ) == 'none' ) {
			$( '#edit_export' ).show();
			data.map.pokemon = prompt( 'Enter pokemon IDs comma separated' );
			data.map.min_level = prompt( 'Enter pokemon minimum level' );
			data.map.max_level = prompt( 'Enter pokemon maximum level' );
			data.map.chance = prompt( 'Enter a chance /100 for pokemon' );
